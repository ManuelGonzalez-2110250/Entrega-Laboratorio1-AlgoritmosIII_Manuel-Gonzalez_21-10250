class ListaAdyacenciaGrafo<T>: Grafo<T> {
   ...
   override fun GradoDeSeparacion(elem1: T, elem2: T):Int{
        var abiertos = ArrayDeque<Camino<T>>()
        var cerrados = mutableSetOf<T>()
        //fallo si no hay elementos
        if(tamanio == 0) return -1
        //si falta alguno de los elementos buscados, retorna
        if(!(contiene(elem1)) || !(contiene(elem2))) return -1
        if(elem1 == elem2) return 0
        //añadimos el primer elemento
        val primerCamino: Camino<T> = Camino()
        primerCamino.agregarAlFinal(elem1)
        abiertos.addLast(primerCamino)
        cerrados.add(elem1)
        //mientras halla caminos abiertos
        while (abiertos.isNotEmpty()){
            //se extrae el último elemento de la cola de caminos
            val vCamino = abiertos.removeFirst()
            //se extrael el último elemento del camino
            val v: T = vCamino.obtenerFinal()!!           
            for(sucesor in obtenerArcosSalida(v)){
                if(sucesor == elem2){
                //si consigo elem2, retorno el tamaño, al no haber colocado aún a elem2 al camino, el tamaño es el adecuado
		    return (vCamino.size)
                }
                //se quiere ver si no fue parte de un camino cerrado
                if(sucesor !in cerrados){
                    cerrados.add(sucesor)
                    var nuevoCamino: Camino<T> = vCamino.copiar()
                    nuevoCamino.agregarAlFinal(sucesor)
                    abiertos.addLast(nuevoCamino)
                }

            }
        }    
        //si terminò sin encontrar nada, retorna -1
        return -1
    }    
}
